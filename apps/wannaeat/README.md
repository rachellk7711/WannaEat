...
- 실제 Supabase Storage에서 버전 포인터(0.3KB)만 먼저 읽고, **버전이 같으면 본문을 다시 받지 않는다**. 새 버전일 때만 카탈로그·규칙을 받아 SHA-256 검증. 연결 실패 시 최근 검증본, 없으면 번들 사용
- 토스에서는 `Storage` SDK, 일반 브라우저에서는 `localStorage`를 이용한 기기 내 기준 저장
- 사진첩 선택·카메라 촬영, **사진 돌리기·보낼 영역 자르기·전송 전 확인**, Edge Function의 원재료 텍스트 추출, 사용자 수정, 고정 규칙 대조와 결과 표시
...
| 값 | 설정 | 어디에 |
|---|---|---|
| 월 예산 | 약 2만원 (`MONTHLY_BUDGET_MICROS = 13_800_000`) | `supabase/functions/extract-ingredients/index.ts` |
| 기기당 하루 | 10장 (`DEVICE_DAILY_LIMIT`) | 같은 파일 |
| IP당 단기 | 10분에 4건 | 같은 파일 |

- 사용량은 `ocr_usage_day` · `ocr_usage_device` 에 쌓인다(`supabase/migrations/`). RLS 정책을 두지 않아 **공개 키로는 읽지도 쓰지도 못하고**, 서버 함수만 `service_role` 로 접근한다.
- 기기는 임의의 UUID(`wannaeat.device.v1`)로만 구분하고, 서버는 그 값을 SHA-256 으로 바꿔 저장한다. 사람을 가리키는 정보가 아니다.
- 한도 확인과 자리 잡기는 SQL 함수 `reserve_ocr_call` 한 번으로 처리한다. 나눠서 확인하면 동시에 들어온 요청이 같은 자리를 두 번 쓴다.
- **사용량을 셀 수 없으면 사진을 보내지 않는다**(503). 예산을 넘기는 쪽보다 잠시 멈추는 쪽을 택한다.
- 같은 사진을 다시 읽지 않는다. 앱이 보낼 픽셀의 SHA-256 을 기억해(`wannaeat.reads.v1`, 최근 20장) 같은 사진이면 서버를 부르지 않고, 한도도 깎이지 않는다.
- 이번 달 사용량과 비용은 `python scripts/22_check_ocr_usage.py` 로 본다.

**예산이 막혔을 때**: 앱은 "이번 달 무료 분석이 모두 끝났어요"를 보여주고 읽기 버튼을 잠근다. 이미 저장된 이력과 기준 설정은 그대로 쓸 수 있다.

## 검증

- `npm test`: 소분류 예외, 개별 해제, 포함 관계, 저장 계약, 잘못된 카탈로그와 검색 회귀 검증
- `npm run lint`, `npm run build`: 정적 검사와 실제 `.ait` 생성
- `tests/e2e-photo.html`: **사진 한 장으로 전체 흐름을 통과시키는 시험** — 기준 선택 → 파일 선택 → 영역 자르기 → 전송 전 확인 → 실제 Edge Function 판독 → 원재료 확인 → 기준 대조 → 이력 저장. 실제 서버를 부르므로 한도를 한 장 쓴다.
